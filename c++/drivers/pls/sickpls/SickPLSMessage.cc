...
namespace SickToolbox {
   ...
   */
  SickPLSMessage::SickPLSMessage( const uint8_t dest_address, const uint8_t * const payload_buffer, const unsigned int payload_length ) :
    SickMessage< SICK_PLS_MSG_HEADER_LEN, SICK_PLS_MSG_PAYLOAD_MAX_LEN, SICK_PLS_MSG_TRAILER_LEN >()  {
   ...
   */
  SickPLSMessage::SickPLSMessage( uint8_t * const message_buffer ) :
    SickMessage< SICK_PLS_MSG_HEADER_LEN, SICK_PLS_MSG_PAYLOAD_MAX_LEN, SICK_PLS_MSG_TRAILER_LEN >()  {
   ...
   * \param payload_length The length of the payload array in bytes
   */
  void SickPLSMessage::BuildMessage( const uint8_t dest_address, const uint8_t * const payload_buffer,
    ...
    _message_buffer[1] = dest_address; // Sick PLS address
        
    /* Include the payload length in the header */
    uint16_t payload_length_16 = host_to_sick_pls_byte_order((uint16_t)_payload_length);
    memcpy(&_message_buffer[2],&payload_length_16,2);

    /*
     * Set the message trailer!
     */    
    _checksum = host_to_sick_pls_byte_order(_computeCRC(_message_buffer,_payload_length+4));
   ...
   */
  void SickPLSMessage::ParseMessage( const uint8_t * const message_buffer ) {
    ...
    _payload_length = (unsigned int)sick_pls_to_host_byte_order(payload_length_16);
    ...
    _checksum = sick_pls_to_host_byte_order(_checksum);
   ...
   */
  void SickPLSMessage::Clear( ) {    

    /* Call the parent method and clear out class' protected members */
    SickMessage< SICK_PLS_MSG_HEADER_LEN, SICK_PLS_MSG_PAYLOAD_MAX_LEN, SICK_PLS_MSG_TRAILER_LEN >::Clear();
   ...
   */
  void SickPLSMessage::Print( ) const {
    ...
    SickMessage< SICK_PLS_MSG_HEADER_LEN, SICK_PLS_MSG_PAYLOAD_MAX_LEN, SICK_PLS_MSG_TRAILER_LEN >::Print();    
  }
   ...
   */
  uint16_t SickPLSMessage::_computeCRC( uint8_t * data, unsigned int data_length ) const {
